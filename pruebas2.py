...
def cargar_laberinto(nombre_archivo):
    ...
    # Convertir cada línea en una lista de caracteres, ignorando líneas vacías
    laberinto = [list(line.strip()) for line in lines if line.strip() != ""]
...
# -------------------------------------------------------
def main():
    print("Seleccione el laberinto a cargar:")
    print("1. Laberinto1.txt")
    print("2. Laberinto2.txt")
    print("3. Laberinto3.txt")
    maze_choice = input("Ingrese el número de la opción deseada: ").strip()
    if maze_choice == "1":
    ...
    grid = cargar_laberinto(maze_file)
    if not grid:
        print("Error: Laberinto vacío o no se pudo cargar.")
        return
    
    print(f"Laberinto cargado: {len(grid)} filas x {len(grid[0])} columnas.")
    
    inicio_pos = encontrar_posiciones(grid, '2')
    salida_pos = encontrar_posiciones(grid, '3')
    ...
    print("Punto de salida:", fin)
    
    print("\nSeleccione el algoritmo a animar:")
    print("1. BFS")
    print("2. DFS")
    print("3. Greedy (Manhattan)")
    print("4. Greedy (Euclidiana)")
    print("5. A* (Manhattan)")
    opcion = input("Ingrese el número de la opción: ").strip()
    
    if opcion == "1":
        generador = bfs_anim(grid, inicio, fin)
        alg = "BFS"
    elif opcion == "2":
        generador = dfs_anim(grid, inicio, fin)
        alg = "DFS"
    elif opcion == "3":
        generador = greedy_anim(grid, inicio, fin, distancia_manhattan)
        alg = "Greedy (Manhattan)"
    elif opcion == "4":
        generador = greedy_anim(grid, inicio, fin, distancia_euclidiana)
        alg = "Greedy (Euclidiana)"
    elif opcion == "5":
        generador = a_star_anim(grid, inicio, fin, distancia_manhattan)
        alg = "A* (Manhattan)"
    else:
    ...
    frame_inicial = crear_frame(grid, set(), [], None)
    ...
    def update(frame):
        ...
        nueva_imagen = crear_frame(grid, explorados, frontier, current, camino)
        im.set_data(nueva_imagen)
        return im,
